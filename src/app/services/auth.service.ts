...
export class AuthService {
  ...
  async getUserRole() {
    try {
      const response: any = await this.http
        .get(this.globalVariable.API_URL2 + '/role')
        .toPromise();
  ...
  getStatus() {
    try {
      return this.http
        .get(this.globalVariable.API_URL2 + '/status')
        .toPromise();
    } catch (error) {
      console.log(error);
      return null;
  ...
  deleteUser(id: number) {
    return this.http
      .delete(this.globalVariable.API_URL2 + '/user/delete' + '/' + id)
      .pipe(retry(3), catchError(this.handleError));
  }

  info(id: number) {
    return this.http
      .get<User>(this.globalVariable.API_URL2 + '/user' + '/' + id)
      .pipe(retry(3), catchError(this.handleError));
  }

  login(user: User) {
    return this.http
      .post<User>(this.globalVariable.API_URL2 + '/login', user)
      .pipe(retry(3), catchError(this.handleError));
  }
  getUsers(): Observable<User[]> {
    return this.http
      .get<User[]>(this.globalVariable.API_URL2 + '/users')
      .pipe(retry(3), catchError(this.handleError));
  }
